...
# 19Sept2019 - Load and explore ADNIMERGE.
#
...
#install.packages( "/home/rstudio/ADNI3/dev-adni3/19Sept2019_Downloads/19Sept2019_ADNIMERGE_0.0.1.tar.gz ", repos = NULL, type = "source" );
...
#   After initial download, do the ADNI search with "Exclude previously downloaded."  This assures that we're looking at net new.
#
loni_list = read.csv( file =  "/home/rstudio/ADNI3/dev-adni3/19Sept2019_Downloads/idaSearch_9_19_2019.csv", header = TRUE, stringsAsFactors = FALSE );
...
qc = read.csv( file =  "/home/rstudio/ADNI3/dev-adni3/19Sept2019_Downloads/19Sept2019_MAYOADIRL_MRI_QUALITY_ADNI3.csv", header = TRUE, stringsAsFactors = FALSE );
dim( qc );
...
if ( 1 ) {
  ...
  df_study_vis_adj = data.frame( df_study %>% dplyr::filter( abs(adnimerge_rec_dist) < 12*30 ) );
  ...
  if ( 0 ) {
    ...
    df = df %>% dplyr::filter( AGE_ON_EXAMDATE >= age_min_mci, AGE_ON_EXAMDATE <= age_max_cn );
      ...
      #
  WriteExcelReport( df, "df_study_adnimerge_19Sept2019.xlsx" );
...
if ( 0 ) {
  ...
  tmp1 = paste( "I", df$image.id.mprage, sep = "" );
  write.table( tmp1, file = "/home/rstudio/ADNI3/misc/re_sched.mprage.txt", row.names = FALSE, col.names = FALSE, quote = FALSE );
  
  tmp2 = paste( "I", df$image.id.rsfmri, sep = "" );
  write.table( tmp2, file = "/home/rstudio/ADNI3/misc/re_sched.rsfmri.txt", row.names = FALSE, col.names = FALSE, quote = FALSE );
  
  tmp3 = data.frame( rid = df$subject.id.mprage, mprage = tmp1, rsfmri = tmp2, dx = df$dx, stringsAsFactors = FALSE ) %>% arrange( rid );
  write.table( tmp3, file = "/home/rstudio/ADNI3/misc/re_sched.19Sept2019.mprage.rsfmri.dx.txt", row.names = FALSE, col.names = FALSE, quote = FALSE );
